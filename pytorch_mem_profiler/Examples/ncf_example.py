...
import sys
sys.path.append('/home/izaak/tbd-tools/pytorch_mem_profiler')
...
def main():
    ...
    run_dir = "./results/{}".format(config['timestamp'])
    ...
    for epoch in range(args.epochs):
        ...
        for batch_index, (user, item, label) in enumerate(loader):
            if batch_index == length//2 and epoch == 0 and PROFILE:
                print('Starting profiling for 100 iterations.')
                cuda.profile_start()

            if batch_index == length//2+100 and epoch == 0 and PROFILE:
        ...
        if args.threshold is not None:
            if np.mean(hits) >= args.threshold:
                print("Hit threshold of {} and saving to ./model/pretrained_ncf_model".format(args.threshold))
                torch.save(model.state_dict(), './model/pretrained_ncf_model')
                return 0
